...
(function ($) {
  ...
  var SeatmapManager = {
    ...
    init: function () {
      ...
      $seatmapBlock.on(
        "click",
        ".mt-calendar-day:not(.mt-calendar-day-disabled)",
    ...
    renderCalendar: function (dates, month, year) {
      ...
      for (var i = 0; i < firstDay; i++) {
        $grid.append(
          '<div class="mt-calendar-day mt-calendar-day-empty"></div>',
        );
      }
      ...
      for (var day = 1; day <= daysInMonth; day++) {
        ...
        if (dateMap[date]) {
          ...
          $day.attr(
            "title",
            mtTicketBus.i18n.unavailableDate || "Unavailable date",
          );
        }
    ...
    selectDate: function (e) {
      ...
      $(".mt-seat-selected").each(function () {
        ...
        if (
          !$seat.hasClass("mt-seat-reserved") &&
          !$seat.hasClass("mt-seat-disabled")
        ) {
    ...
    updateCourseAvailability: function (coursesAvailability) {
      ...
      $timeOptions.each(function () {
        ...
        if (courseInfo) {
          var availableText =
            mtTicketBus.i18n.availableSeats || "available seats";
    ...
    renderSeatMap: function (layoutData, availableSeats) {
      ...
      html +=
        ...
        (mtTicketBus.i18n.available || "Available") +
        "</span>";
      html +=
        '<span class="mt-legend-item"><span class="mt-legend-seat mt-seat-reserved"></span> ' +
        (mtTicketBus.i18n.reserved || "Reserved") +
        "</span>";
      html +=
        '<span class="mt-legend-item"><span class="mt-legend-seat mt-seat-selected"></span> ' +
        (mtTicketBus.i18n.selected || "Selected") +
        "</span>";
      html +=
        '<span class="mt-legend-item"><span class="mt-legend-seat mt-seat-disabled"></span> ' +
        (mtTicketBus.i18n.disabled || "Disabled") +
        "</span>";
      html += "</div>";
    ...
    renderSeat: function (seatId, seats, availableSeats) {
      var isSeatEnabled =
        seats[seatId] === true || seats[seatId] === 1 || seats[seatId] === "1";
    ...
    selectSeat: function (e) {
      ...
      if (
        !$seat.hasClass("mt-seat-available") &&
        !$seat.hasClass("mt-seat-selected")
      ) {
