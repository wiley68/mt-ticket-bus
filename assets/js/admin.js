...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    function showPageLoading() {
      ...
      var container = $(
        ".mt-buses-container, .mt-routes-container, .mt-schedules-container",
      );
    ...
    $(document).on(
      "click",
      ".mt-ticket-bus-buses .page-title-action",
      function (e) {
    ...
    $(document).on(
      "click",
      ".mt-ticket-bus-routes .page-title-action",
      function (e) {
        // Check if link doesn't contain edit parameter (it's a "New" button)
        if ($(this).attr("href").indexOf("edit=") === -1) {
          // Show loading overlay immediately
    ...
    $(document).on(
      "click",
      ".mt-ticket-bus-schedules .page-title-action",
      function (e) {
        // Check if link doesn't contain edit parameter (it's a "New" button)
        if ($(this).attr("href").indexOf("edit=") === -1) {
          // Show loading overlay immediately
    ...
    // Check if new course starts after the start of an existing course
    function coursesOverlap(course1, course2) {
      ...
      // For validation we always pass the new course as course1 and
      // an existing course as course2. We only want to forbid cases
      // where the new course starts at or before the start of an
      // existing course.
      return dep1 <= dep2;
    }
    ...
    function validateCourse(departureTime, arrivalTime) {
      if (!departureTime || !arrivalTime) {
        return {
          valid: false,
          message: "Both departure and arrival times are required.",
        };
      }
      ...
      // Check that the new course starts after the start of existing courses
      for (var i = 0; i < coursesData.length; i++) {
        if (coursesOverlap(newCourse, coursesData[i])) {
          return {
            valid: false,
            message:
              "The departure time of this course must be after the start of the previous course (" +
              coursesData[i].departure_time +
              " - " +
              coursesData[i].arrival_time +
              ").",
          };
        }
      }
    ...
    $(document).on("click", ".mt-remove-course", function () {
      ...
      coursesData = coursesData.filter(function (course) {
        return (
          course.departure_time !== departureTime ||
          course.arrival_time !== arrivalTime
        );
      });
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            var newUrl =
              currentUrl.split("&saved=")[0].split("?saved=")[0] +
              separator +
              "saved=1";
            ...
            var errorMsg =
              response.data && response.data.message
                ...
                : mtTicketBusAdmin.i18n.errorOccurredSavingSchedule;
    ...
    $(document).on("click", ".mt-schedule-info", function (e) {
      ...
      htmlContent +=
        "<p><strong>" +
        mtTicketBusAdmin.i18n.scheduleName +
        ":</strong> " +
        name +
        "</p>";
      htmlContent +=
        "<p><strong>" +
        mtTicketBusAdmin.i18n.scheduleRoute +
        ":</strong> " +
        route +
        "</p>";
      htmlContent +=
        "<p><strong>" +
        mtTicketBusAdmin.i18n.scheduleCourses +
        ":</strong> " +
        courses +
        "</p>";
      htmlContent +=
        "<p><strong>" +
        mtTicketBusAdmin.i18n.scheduleFrequency +
        ":</strong> " +
        frequency +
        "</p>";
      htmlContent +=
        "<p><strong>" +
        mtTicketBusAdmin.i18n.scheduleStatus +
        ":</strong> " +
        status +
        "</p>";
      htmlContent += "</div>";
          ...
          if (parsed && parsed.seats && typeof parsed.seats === "object") {
            ...
            for (var seatId in parsed.seats) {
              if (parsed.seats.hasOwnProperty(seatId)) {
                seatLayoutData[seatId] =
                  parsed.seats[seatId] === true ||
                  parsed.seats[seatId] === "true" ||
                  parsed.seats[seatId] === 1;
              }
            }
          }
        } catch (e) {
    ...
    function generateSeatLayout() {
      ...
      if (leftSeats === 0 && rightSeats === 0) {
        $("#mt-seat-layout-container").html(
          '<p class="description">Please configure seat columns first.</p>',
        );
        $("#seat_layout").val("");
        $("#total_seats").val(0);
        return;
      ...
      for (var row = 1; row <= rows; row++) {
        ...
        for (var l = 0; l < leftSeats; l++) {
          ...
          if (seatElement.length) {
            ...
            currentSeatData[seatId] =
              seatLayoutData[seatId] !== undefined
                ? seatLayoutData[seatId]
                : true;
          }
        }
    ...
    $("#left_column_seats, #right_column_seats, #number_of_rows").on(
      "change",
      function () {
        ...
        success: function (response) {
          if (response.success) {
            ...
            var errorMsg =
              response.data && response.data.message
                ? response.data.message
                : mtTicketBusAdmin.i18n.errorOccurredSaving;
            Swal.fire({
              icon: "error",
              title: errorMsg,
              confirmButtonText: mtTicketBusAdmin.i18n.ok,
            ...
            if (
              errorMsg.indexOf("registration number") !== -1 ||
              errorMsg.indexOf("Registration number") !== -1
            ) {
    ...
    function initializeStations() {
      ...
      if (stationsData.length === 0) {
        $("#stations_list .mt-station-badge:not(.empty-state)").each(
          function () {
            var badge = $(this);
    ...
    function validateStation(stationName, stationDuration, totalDuration) {
      ...
      if (stationsData.length > 0) {
        ...
        if (duration <= lastDuration) {
          return {
            valid: false,
            message:
              "Duration must be greater than the previous station (" +
              lastDuration +
              " minutes).",
          };
        }
      ...
      if (!isNaN(totalDur) && totalDur > 0 && duration >= totalDur) {
        return {
          valid: false,
          message:
            "Station duration must be less than the total route duration (" +
            totalDur +
            " minutes).",
        };
      }
    ...
    function validateRouteForm() {
      ...
      if (stationsData.length > 0) {
        var lastStationDuration =
          stationsData[stationsData.length - 1].duration;
        if (totalDuration <= lastStationDuration) {
          return {
            ...
            message:
              ...
              totalDuration +
              " minutes) must be greater than the last intermediate station duration (" +
              lastStationDuration +
              " minutes).",
          };
        }
          ...
          for (var i = 0; i < stationsData.length; i++) {
            if (stationsData[i].duration >= totalDuration) {
              ...
              invalidMessage =
                ...
                stationsData[i].name +
                '" has duration (' +
                stationsData[i].duration +
                " min) greater than or equal to total route duration (" +
                totalDuration +
                " min).";
              break;
            }
        ...
        success: function (response) {
          if (response.success) {
            ...
            var errorMsg =
              response.data && response.data.message
                ? response.data.message
                : mtTicketBusAdmin.i18n.errorOccurredSavingRoute;
            Swal.fire({
              icon: "error",
              title: errorMsg,
              confirmButtonText: mtTicketBusAdmin.i18n.ok,
    ...
    // Handle extra deletion (confirm then redirect)
    $(document).on("click", ".mt-delete-extra", function (e) {
      e.preventDefault();
      ...
      Swal.fire({
        icon: "warning",
        title: mtTicketBusAdmin.i18n.confirmDeleteExtra,
        showCancelButton: true,
        confirmButtonText: mtTicketBusAdmin.i18n.yes,
        cancelButtonText: mtTicketBusAdmin.i18n.cancel,
        confirmButtonColor: "#d33",
  ...
  if ($(".mt-ticket-bus-reservations").length) {
    ...
    function renderReservationsSeatMap() {
      ...
      var layoutData =
        typeof seatLayoutJson === "string"
          ? JSON.parse(seatLayoutJson)
          : seatLayoutJson;
      var reservedSeats =
        typeof reservedSeatsJson === "string"
          ? JSON.parse(reservedSeatsJson)
          : reservedSeatsJson || [];
    ...
    function showReservationDetails(reservation) {
      ...
      if (reservation.order_id) {
        ...
        html +=
          ...
          (mtTicketBusAdmin.i18n.orderId || "Order ID") +
          ":</strong> ";
        html +=
          '<a href="' +
          escapeHtml(orderEditUrl) +
          '">#' +
          orderId +
          "</a>" +
          escapeHtml(orderDateText);
        html += "</div>";
      }
      ...
      if (reservation.product_name) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.productName || "Ticket") +
          ":</strong> " +
          escapeHtml(reservation.product_name);
        html += "</div>";
      }
      ...
      if (reservation.order_status) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.orderStatus || "Order Status") +
          ":</strong> " +
      ...
      if (reservation.payment_method) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.paymentMethod || "Payment Method") +
          ":</strong> " +
          escapeHtml(reservation.payment_method);
        html += "</div>";
      }
      ...
      if (reservation.hasOwnProperty("order_notes")) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.orderNotes || "Order Notes") +
          ":</strong> ";
        if (reservation.order_notes && reservation.order_notes.trim()) {
          // Split notes by newlines and format each on a new line with better spacing
          var notesArray = reservation.order_notes
            .split("\n")
            .filter(function (note) {
              return note.trim().length > 0;
            });

          // Reverse array to show oldest notes first (they were added in reverse chronological order)
          notesArray.reverse();

          if (notesArray.length > 0) {
            html += '<div style="margin-top: 0.5em; padding-left: 0;">';
            notesArray.forEach(function (note, index) {
              html +=
                '<div style="margin-bottom: ' +
                (index < notesArray.length - 1 ? "0.5em" : "0") +
                ';">';
              html +=
                "<strong>" +
                (index + 1) +
      ...
      if (reservation.seat_number) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.seatNumber || "Seat Number") +
          ":</strong> " +
          escapeHtml(reservation.seat_number);
        html += "</div>";
      }
      ...
      if (reservation.passenger_name) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.passengerName || "Passenger Name") +
          ":</strong> " +
          escapeHtml(reservation.passenger_name);
        html += "</div>";
      }
      ...
      if (reservation.passenger_email) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.passengerEmail || "Passenger Email") +
          ":</strong> ";
      ...
      if (reservation.passenger_phone) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.passengerPhone || "Passenger Phone") +
          ":</strong> ";
      ...
      if (reservation.departure_date) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.departureDate || "Departure Date") +
          ":</strong> " +
          escapeHtml(reservation.departure_date);
        html += "</div>";
      }
      ...
      if (reservation.departure_time) {
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.departureTime || "Departure Time") +
          ":</strong> " +
      ...
      if (reservation.status) {
        ...
        if (reservation.status === "reserved") {
          ...
          statusLabel = mtTicketBusAdmin.i18n.statusReserved || "Reserved";
        } else if (reservation.status === "confirmed") {
          statusClass = "confirmed";
          statusLabel = mtTicketBusAdmin.i18n.statusConfirmed || "Confirmed";
        } else if (reservation.status === "cancelled") {
          statusClass = "cancelled";
          statusLabel = mtTicketBusAdmin.i18n.statusCancelled || "Cancelled";
        } else {
          // Fallback: capitalize first letter for unknown statuses
          statusLabel =
        ...
        html +=
          '<strong style="white-space: nowrap;">' +
          (mtTicketBusAdmin.i18n.status || "Status") +
          ":</strong> ";
        html +=
          '<span class="mt-reservation-status mt-status-' +
